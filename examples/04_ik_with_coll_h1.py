...
def main():
    ...
    target_link_name = "right_jaw_right_finger_link"
    # robot = pk.Robot.from_urdf(urdf)
    # Resolve URDF path: examples -> pyroki -> control -> src
    _src_dir = Path(__file__).resolve().parent.parent.parent.parent
    urdf_path = _src_dir / "data" / "urdf" / "zerith" / "ZR_H1PRO_collision.urdf"
    if not urdf_path.exists():
        urdf_path = Path("data/urdf/zerith/ZR_H1PRO_collision.urdf")
    urdf = URDF.load(str(urdf_path))
    robot = pk.Robot.from_urdf(urdf)

    # Create capsule collision model (works for any URDF).
    robot_coll_capsule = RobotCollision.from_urdf(urdf)

    # Optionally load sphere decomposition (must match this robot's link names).
    # Generated via https://github.com/chungmin99/ballpark
    robot_coll_sphere = None
    sphere_json_path = Path(__file__).parent / "assets" / "panda_spheres.json"
    if sphere_json_path.exists():
        ...
        try:
            ...
            if "No spheres found" in str(e):
                print("Warning: Sphere decomposition doesn't match this robot's link names. Using capsule only.")
            else:
                raise
    ...
    coll_options = ("Capsule", "Sphere") if robot_coll_sphere is not None else ("Capsule",)
    coll_type_handle = server.gui.add_dropdown(
        "Collision Type", coll_options, initial_value="Capsule"
    )
    ...
    while True:
        ...
        robot_coll = (
            robot_coll_sphere
            if robot_coll_sphere is not None and coll_type_handle.value == "Sphere"
